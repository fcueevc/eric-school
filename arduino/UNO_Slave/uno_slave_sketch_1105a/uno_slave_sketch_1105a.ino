...
#define gpio_led3 11 
#define gpio_led4 12
#define gpio_led5 4     // 對錯燈號 high動作
...
void setup() {
  ...
  digitalWrite(gpio_led5, LED_OFF);    // 開機設定為LOW
  // 音效腳位(輸出)
  ...
  digitalWrite(gpio_K1, K_OFF);    // 開機設定為LOW, 設定
  digitalWrite(gpio_K2, K_OFF);    // 開機設定為LOW, 正確
//  digitalWrite(gpio_K3, K_OFF);    // 開機設定為LOW, 錯誤
  digitalWrite(gpio_K4, K_OFF);    // 開機設定為LOW, 未使用

  //Serial.print("AT+RQA(1,1)\r");      
...
void loop() {
  ...
  if(Serial.available() >0)
  {
     ...
     // remove any \r \n whitespace at the end of the String
      ...
      strTemp1  = strOK + SLAVE_ID + ",1)";
      strTemp2  = strOK + SLAVE_ID + ",2)";
      strTemp3  = strOK + SLAVE_ID + ",3)";
      ...
      {      
        ...
        digitalWrite(gpio_led5, LED_ON);    // 對錯燈 正確常亮
      ...
      else if(teststr=="AT+RESET")
      {
        ...
        digitalWrite(gpio_led5, LED_OFF);
       // digitalWrite(gpio_K3, K_OFF);     

      }
      else if((teststr==strTemp1)||(teststr==strTemp2)||(teststr==strTemp3))
      {
        //Serial.println(strTemp);
        ...
        digitalWrite(gpio_led5, !LED_ON);  // 對錯燈 錯誤閃爍 
        delay(500);      
      }
  }
  ...
  if((digitalRead(gpio_set)==GPIO_PRESS)&&(set_flag==0)&&(leds!=0))
  {
     ...
     // 詢問答案 AT+RQA(s,a)<CR>
     // Response OK+(s,n)<CR><LF>
     // s:學習站ID, a:學習站答案, n:答案
     Serial.print(strRQA);      // AT+RQA(
     Serial.print(SLAVE_ID);    // s: ID
     Serial.print(",");         
     Serial.print(answerbuf);   // a: answer
     Serial.print(strEND);      // )<CR>
  }
  else if((digitalRead(gpio_set)==GPIO_PRESS)&&(set_flag==1))
  {
     ...
     digitalWrite(gpio_led3, LED_OFF);
     digitalWrite(gpio_led4, LED_OFF);
     digitalWrite(gpio_led5, K_OFF);     // 答題LED OFF(LOW) 
     delay(500);
     while(digitalRead(gpio_set)==GPIO_PRESS);
     delay(100);
  }
